...
| `v2/plantillas/` | Tres plantillas: `MAPA_DE_CARPETAS.md` (la sección *Mapa de carpetas* que se pega al final del `CLAUDE.md` de la raíz: estructura, dónde va cada cosa; no es un router alternativo), `CLAUDE_brain.md` (gobernanza del segundo cerebro: notas planas, wikilinks, índice, ingest diferencial) y `settings_dell.json` (los settings de usuario de la máquina de trabajo: modelo, techo de contexto, statusline y los hooks de arranque y contador). |
| `hooks/` | `contador_turnos.sh` (contador de turnos), `arranque_estado.sh` (inyecta ESTADO_ACTUAL.md al arrancar) y `statusline_costo.sh` (costo, contexto y caché abajo del prompt). |
...
1. **Clonar** el repo en la máquina de trabajo: `git clone https://github.com/groucooliva/kit-claude.git` (es público: no pide login). El detalle de ida y vuelta está en `SYNC.md`. El repo trae `.gitattributes` para que los `.sh` lleguen con fin de línea LF aunque Git for Windows esté en `autocrlf=true`; si un clon es anterior a ese archivo, `git pull` y después `git rm --cached -r . && git reset --hard` (o reclonar).
2. **Apuntar el proyecto al kit.** En Claude Code: copiar `v2/CLAUDE.md` a `CLAUDE.md` en la raíz del directorio de trabajo (o a `~/.claude/CLAUDE.md` para que valga en todos), con `QUIEN_SOY.md`, `HANDOFF.md`, `PRINCIPIOS_DE_TRABAJO.md` y `COMO_TRABAJAMOS.md` al lado. **Ese es el único router**: no hay otro `CLAUDE.md` de raíz que armar. En la interfaz web: pegar `PRINCIPIOS_DE_TRABAJO.md` en las instrucciones del proyecto y subir el resto al knowledge.
...
4. **Armar la estructura de carpetas** con `v2/plantillas/`: pegar `MAPA_DE_CARPETAS.md` (completado) al final del `CLAUDE.md` del paso 2, y `CLAUDE_brain.md` como `CLAUDE.md` dentro de la carpeta del segundo cerebro. Claude Code se abre siempre desde la raíz, nunca desde una subcarpeta.
