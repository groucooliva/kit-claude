...
5. **Las skills no se tocaron.** Si algún día cambian, cambian en `../skills/` y esta carpeta no se entera.
...
Tres vías previstas, en orden de preferencia. **Ninguna está ejecutada todavía: nada de esto está subido a ningún lado.**

**1. Repo público de GitHub.** Publicar la carpeta completa del kit (v1, v2 y `skills/`) en un repo propio. Desde la máquina de trabajo: `git clone`, o traer un archivo suelto por su URL raw. Ventaja: se actualiza en un lugar y se baja donde haga falta, y no requiere mandarse nada por mail. Condición: el checklist de arriba tiene que estar cerrado, porque público es público.
