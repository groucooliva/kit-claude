...
**1. Copiar los tres scripts** de `hooks/` a `~/.claude/hooks/`. Sin `chmod`: se invocan como `bash <ruta>`, que no necesita bit de ejecución. Los tres usan solo builtins de bash (sin jq, sed ni find) porque Git for Windows no trae jq y el hook puede correr con un PATH pelado.
...
- `"statusLine"`: `statusline_costo.sh` muestra abajo del prompt modelo, costo de la sesión en USD, % de contexto, estado del caché (warm/fría, TTL, hit) y el turno. Cero tokens. En Enterprise no hay barras de plan; el costo es a precio de lista salvo que el admin haya fijado `modelPricing`. Se resetea con `/clear`.
...
- `hooks.UserPromptSubmit`: `contador_turnos.sh`, el contador de turnos. Avisa en pantalla (`systemMessage`) y al modelo (`additionalContext`) en el turno 1, 15, 25 y desde el 35 cada 5.
